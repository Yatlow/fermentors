import { dayOfWeek, parseDate, weekStart, type Settings, type WeekPlan } from "./planningEngine";
import { packagingLimit } from "./productionCycle";
...
export function validatePlanningWeek(
  ...
  for (const r of w.packaging) {
    ...
    if (!w.allowExceptions && r.date && dayOfWeek(r.date) > 4) return "אין אריזה רגילה בשישי או שבת";
    if (r.date) {
      ...
      if (!w.allowExceptions && qty > packagingLimit(r.date, p.type))
        return dayOfWeek(r.date) === 0 ? "בראשון: עד 168 ארגזים או 100 חביות" : "עד 252 ארגזים ביום";
  ...
  for (const d of w.deliveries ?? []) {
    if (!settings.products.some((p) => p.id === d.productId) || !Number.isInteger(d.quantity) || d.quantity <= 0 || !parseDate(d.dispatchDate) || !parseDate(d.arrivalDate) || weekStart(d.dispatchDate) !== w.id || d.arrivalDate < d.dispatchDate)
  ...
  for (const b of w.brews) {
    if (!w.allowExceptions && (dayOfWeek(b.date) < 1 || dayOfWeek(b.date) > 3)) return "בישול משובץ בימים שני–רביעי בלבד";
