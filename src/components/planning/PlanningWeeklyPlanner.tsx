    ...
    <div className="bp-week-recommendations">
      <article className="bp-week-rec-card bp-week-shipment-card">
        ...
        <div className="bp-shipment-plan-table"><div className="bp-shipment-plan-head"><span>מקט</span><span>כיסוי</span><span>מומלץ</span><span>נקבע</span></div>{products.map((p) => {
          ...
          return <div className={`bp-shipment-plan-row ${risk ? "is-warning" : "is-ok"} ${mismatch ? "gap-warning" : ""}`} key={p.id}><span><b>{displayStyle(p.style)}</b><small>{p.type === "crates" ? "ארגזים" : "חביות"}</small></span><span>{cover(base?.tempoCover)}<small>מבשלה צפויה: {fmt(base?.breweryUnits ?? 0)}</small></span><span>{rec?.pallets ?? 0} משטחים<small>{rec?.slots ?? 0} מק׳</small></span><span>{editing === "delivery" ? <div className="bp-stepper"><button onClick={() => stepShipment(p,-1)}>−</button><b>{Math.round((shipDraft[p.id] ?? 0)/palletSize(p))}</b><button onClick={() => stepShipment(p,1)}>+</button><small>משטחים</small></div> : <>{decidedPallets} משטחים<small>אחרי החלטה: {cover(after?.tempoCover)}</small></>}</span></div>;
        })}</div>
