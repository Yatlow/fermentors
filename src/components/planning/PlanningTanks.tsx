...
export default function PlanningTanks({ tanks, sources, plans, settings, actuals, today }: {
  ...
  const rows = sources
    ...
    .map((source) => {
      ...
      const next = brews.filter((b) => b.tankId === source.id).sort((a, b) => a.date.localeCompare(b.date))[0];
      const plannedEmpty = plans.flatMap((w) => w.packaging)
        .filter((p) => p.tankId === source.id && p.date && p.date >= today)
        .sort((a, b) => a.date!.localeCompare(b.date!)).at(-1)?.date;
    ...
    <section>
      ...
      <div className="bp-table-scroll">
        <table className="bp-stock-table bp-tank-table">
          <thead><tr><th>מיכל</th><th>תכולה נוכחית</th><th>מילוי נוכחי</th><th>הבשלה</th><th>ריקון</th><th>זמין לבישול</th><th>בישול הבא</th></tr></thead>
