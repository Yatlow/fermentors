...
export default function PlanningBoard({ settings, plans, tanks, brews, actuals, shipments, today, holidays, workspace, disabled, saveWeek }: {
  ...
  const recommendations = actions.filter((a) => {
    if (a.kind !== "packaging") return true;
    return !current.packaging.some((p) => p.date === a.date);
  });
  ...
  async function persist(next: WeekPlan) {
    ...
    const error = validatePlanningWeek(next, settings, [...plans.filter((w) => w.id !== next.id), next], today);
    ...
    await saveWeek(next);
    setDraft(null); setEditingDay(null); setMessage("ההחלטות נשמרו");
  }

  async function hide(id: string) {
    setBusy(true); setMessage("");
    try {
      await saveWeek({ ...current, dismissedRecommendations: [...new Set([...(current.dismissedRecommendations ?? []), id])], changeReason: "דילוג על המלצה" });
  ...
  function editAction(action: PlanningAction) {
    ...
    const base = workspace.effectivePlans.find((w) => w.id === targetWeek) ?? { ...emptyWeek(targetWeek), maxRuns: settings.preferredRuns };
  ...
  function editNeed(need: ProductionNeed) {
    ...
    if (need.kind === "brew") next.brews.push({ id: crypto.randomUUID(), style: need.style, tankId: candidate?.id ?? "", date, liters: Math.min(need.quantity, candidate?.liters ?? need.quantity) });
      ...
      const available = candidate ? Math.floor(candidate.liters / litersPerUnit(product)) : need.quantity;
      next.packaging.push({ id: crypto.randomUUID(), productId: product.id, tankId: candidate?.id ?? "", quantity: Math.max(1, Math.min(need.quantity, available, packagingLimit(date, product.type))), date, source: "manual" });
    }
  ...
  return <section>
    ...
    <div className="bp-week-picker">
      {Array.from({ length: horizon }, (_, i) => addDays(weekStart(today), i * 7)).map((w) => <button key={w} aria-pressed={week === w} disabled={!!draft} onClick={() => { setWeek(w); setPickupDay(""); setRecommendationIndex(0); }}>שבוע {weekNumber(w)}<small><bdi>{shortDate(w)}</bdi></small></button>)}
    </div>
    ...
    <div className="bp-calendar">{days.map((day, i) => {
      ...
      const packs = current.packaging.filter((r) => r.date === date); const brewing = current.brews.filter((b) => b.date === date);
      const deliveries = (effective.deliveries ?? []).filter((d) => d.dispatchDate === date); const done = actuals.filter((a) => actualDate(a) === date); const sent = shipments.filter((s) => s.date === date);
      return <article className={`bp-day ${date === today ? "bp-day-today" : ""}`} key={date}><h3>{day}{date === today ? " · היום" : ""}<small>{shortDate(date)}</small></h3>
        ...
        {holidays.filter((h) => h.date === date).map((h) => <p className="bp-holiday" key={h.title}>{h.title}</p>)}
        {(current.deliveryDates ?? []).includes(date) && <p>נקבע איסוף לטמפו</p>}
        ...
        {done.map((a) => <div className="bp-completed" key={a.id}><small>בוצע</small><b>{a.beerStyle ?? "אריזה"}</b></div>)}
        {sent.map((s, idx) => <div className="bp-completed" key={`${s.date}:${idx}`}><small>בוצע · משלוח</small><b>{s.quantity ?? ""}</b></div>)}
      </article>;
    })}</div>
