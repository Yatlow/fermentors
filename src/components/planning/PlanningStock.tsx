    ...
    return (
        <section>
        <div className="bp-section-heading"><div><p className="bp-muted">המלאי בטמפו מוצג לפי הנתונים שמוזנים בטאב הזנת נתוינים. מלאי מבשלה מציג את מה שמוזן במפת מקרר (כולל חדר בקבוקים, בהעמסה למשלוח וממתינים לשיבוץ)</p></div></div>
        <div className="bp-stock-snapshot">
            ...
            return (
                ...
                <div className={`bp-stock-style ${beerStyleClass(g.style).className}`}>{displayStyle(g.style)}
                    <small className="bp-stock-expand-hint">{open ? "סגירה ▴" : "פרטים נוספים ▾"}</small></div>
                {[crates, kegs].map((f) => (
                ...
                {open && (
                    ...
                    {g.formats.map((f) => (
                        ...
                        {f.tempo !== null && f.daily > 0 && <small>בקצב הנוכחי הכיסוי הכולל יספיק עד {shortDate(addDays(today, f.totalCover.days))}</small>}
                        </div>
                    ))}
                    <div><b>פעולות קרובות</b>
                    {nextActions.filter((a) => a.group === g.key)
                    .slice(0, 6).map((a, i) => 
                    <span key={`${a.date}:${a.kind}:${i}`}>
                        {shortDate(a.date)} · {a.kind==="packaging"? "אריזה":a.kind==="brew"?"בישול":"משלוח"} - {a.description} · <b className={`bp-status-chip ${statusClass(a.status)}`}>{a.status}</b></span>)}{!nextActions.some((a) => a.group === g.key) && <span>אין פעולות קרובות</span>}</div>
                    </div>
                )}
                </article>
            );
