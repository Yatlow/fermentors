...
export default function PlanningWeeklyRecommendations({
  ...
  const warnings = useMemo(() => {
    ...
    const result = unresolved.map((n) => `${displayStyle(n.style)}: יש צורך צפוי ב${n.kind === "brew" ? "בישול" : "אריזה"}, אבל כרגע אין שיבוץ ישים. ${n.problem}`);
    const engine = workspace.forecast.warnings.filter((w) =>
      [week, ...Array.from({ length: 7 }, (_, i) => shortDate(addDays(week, i)))].some((token) => w.includes(token)),
    );
  ...
  const sourceNumber = (id: string) =>
    sources.find((s) => s.id === id)?.tankNumber ?? tanks.find((t) => t.id === id)?.number ?? id;
  ...
  const shippingLines = useMemo(() => {
    ...
    for (const action of ship) {
      ...
      const old = groups.get(key) ?? {
        label: displayStyle(p.style), pallets: 0, quantity: 0,
  ...
  const packagingLines = useMemo(() => {
    return pack.flatMap((action) => {
      ...
      if (!allocations.length)
        return [{ key: action.id, text: `${displayStyle(p.style)} · ${fmt(action.quantity)} ${p.type === "crates" ? "ארגזים" : "חביות"}`, sub: "לא נמצא כרגע מיכל ישים — זו אזהרה ולא המלצה לביצוע", warning: true }];
      return [{
        key: action.id,
        ...
        sub: allocations.map((a) => {
          ...
          const nextWeek = workspace.actions.some((x) =>
            x.kind === "packaging" && weekStart(x.date) === addDays(week, 7) &&
  ...
  const brewingLines = brew.flatMap((action) => {
    ...
    return [{
      ...
      sub: action.dependent ? "זמין לאחר ריקון מתוכנן" : "המיכל פנוי / מחכה לבישול",
    }];
  });
  ...
  async function accept(kind: Kind) {
    ...
    try {
      ...
      for (const action of selected) {
        const already = action.kind === "brew"
  ...
  const savedDeliveryPallets = (current.deliveries ?? []).reduce((sum, d) => sum + (d.pallets?.length ?? 0), 0);

  return (
    <section className="bp-weekly-planner">
      <div className="bp-section-heading"><div><h2>המלצות שבועיות</h2><p className="bp-muted">המתכנן מחליט מה צריך לקרות השבוע. השיבוץ ליום מסוים נעשה אחר כך בלוח העבודה היומי.</p></div></div>
      <div className="bp-week-picker">
        {Array.from({ length: 8 }, (_, i) => addDays(weekStart(today), i * 7)).map((w) => (
          <button key={w} aria-pressed={week === w} onClick={() => setWeek(w)}>שבוע {weekNumber(w)}<small>{shortDate(w)}</small></button>
        ))}
      </div>
      {warnings.length > 0 && <div className="bp-week-alerts"><b>אזהרות לתוכנית</b>{warnings.map((warning) => <span key={warning}>⚠ {warning}</span>)}</div>}
      ...
      <div className="bp-week-recommendations">
        <article className="bp-week-rec-card">
          ...
          {shippingLines.length ? shippingLines.map((line) => <div className="bp-rec-line" key={`${line.label}:${line.unit}`}><b>{line.label}</b><span>{line.pallets} משטחים · {fmt(line.quantity)} {line.unit}</span></div>) : <p className="bp-muted">אין צורך במשלוח נוסף לפי התחזית לשבוע הזה.</p>}
        ...
        <article className="bp-week-rec-card">
          ...
          {packagingLines.length ? packagingLines.map((line) => <div className={`bp-rec-line ${line.warning ? "is-warning" : ""}`} key={line.key}><b>{line.text}</b><span>{line.sub}</span></div>) : <p className="bp-muted">אין המלצת אריזה נוספת לשבוע הזה.</p>}
        ...
        <article className="bp-week-rec-card">
          ...
          {brewingLines.length ? brewingLines.map((line) => <div className="bp-rec-line" key={line.key}><b>{line.text}</b><span>{line.sub}</span></div>) : <p className="bp-muted">אין כרגע בישול נוסף שנדרש לפי התחזית. מיכל שמחכה לבישול מקבל נפח משוער אוטומטי לפי גודלו כדי שיוכל להשתתף בהמלצה.</p>}
