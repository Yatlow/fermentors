...
export default function PlanningWeeklyRecommendationsV2({ settings, plans, tanks, sources, pallets, actuals, shipments, holidays, today, disabled, saveWeek }: {
  ...
  function stepShipment(p: Product, delta: number) {
    ...
    setShipDraft((prev) => {
      ...
      if (delta > 0 && proposed > maxShipmentQty(p)) {
        setMessage(`אין מספיק מלאי זמין עד השבוע הזה עבור ${displayStyle(p.style)}. זמין: ${Math.floor(maxShipmentQty(p) / step)} משטחים.`);
        return prev;
      }
  ...
  function allDraftPackLines() {
    ...
    for (const r of current.packaging) {
      ...
      const quantity = Math.max(0, packDraft[key] ?? r.quantity);
  ...
  function remainingLitersForTank(tankId: string, excludeManualId?: string) {
    ...
    const used = [
      ...current.packaging.map((r) => ({ id: `saved:${r.id}`, tankId: r.tankId, productId: r.productId, quantity: Math.max(0, packDraft[r.id ?? `${r.productId}:${r.tankId}`] ?? r.quantity) })),
  ...
  async function savePackaging() {
    ...
    try {
      ...
      for (const manual of manualPacks) {
        ...
        const usedForTank = packaging.filter((x) => x.tankId === t.id).reduce((sum, x) => sum + x.quantity * litersPerUnit(product(x.productId)!), 0);
  ...
  return <section className="bp-weekly-planner">
    ...
    <div className="bp-week-recommendations">
      <article className="bp-week-rec-card bp-week-shipment-card">
        ...
        <p className="bp-rec-principle">הכיסוי הוא כיסוי טמפו צפוי לסוף השבוע. "זמין למשלוח" כולל מלאי פיזי שקיים היום וגם מלאי שכבר צפוי להיווצר מתכנון קודם.</p>
        ...
        <div className="bp-shipment-plan-table">
          ...
          {shipmentProducts.map((p) => {
            ...
            const physical = physicalInventory(p); const expected = base?.breweryUnits ?? 0; const plannedDelta = expected - physical;
            const shownAfter = editing === "delivery" ? draftShipmentCover(p) : after?.tempoCover ?? null;
            return <div className={`bp-shipment-plan-row ${(shownAfter ?? Infinity) < settings.targetWeeks ? "is-warning" : "is-ok"}`} key={p.id}>
              <span><b>{displayStyle(p.style)}</b><small>{p.type === "crates" ? "ארגזים" : "חביות"}</small></span>
              <span>{coverLabel(base?.tempoCover ?? null)}<small>זמין למשלוח: {fmt(expected)} · פיזי היום {fmt(physical)}{plannedDelta > 0 ? ` · +${fmt(plannedDelta)} מתכנון` : plannedDelta < 0 ? ` · ${fmt(plannedDelta)} מהחלטות קודמות` : ""}</small></span>
              <span>{rec?.pallets ?? 0} מש׳<small>{rec?.slots ?? 0} מק׳</small></span>
              <span>{editing === "delivery" ? <div className="bp-stepper"><button onClick={() => stepShipment(p, -1)}>−</button><b>{Math.round((shipDraft[p.id] ?? 0) / palletSize(p))}</b><button onClick={() => stepShipment(p, 1)}>+</button><small>משטחים · אחרי: {coverLabel(shownAfter)}</small></div> : <>{decidedPallets} מש׳<small>אחרי החלטה: {coverLabel(shownAfter)}</small></>}</span>
            </div>;
          })}
        </div>
      ...
      <article className="bp-week-rec-card">
        ...
        <p className="bp-rec-principle">המלצת המערכת לעולם לא מציעה יותר מ־252 ארגזים בריצת אריזה אחת. למתכנן מותר לחרוג ידנית. יתרת המיכל מחושבת מחדש אחרי כל שורה שהוספת.</p>
        <div className="bp-shipment-plan-table"><div className="bp-shipment-plan-head"><span>מקט</span><span>כיסוי כולל</span><span>מומלץ</span><span>החלטה</span></div>
          {packagingProducts.map((p) => {
            const before = model.rows.afterShipment.get(p.id); const after = model.rows.afterPackaging.get(p.id); const rec = packagingRecByProduct.get(p.id) ?? 0; const decided = currentPackagingQty(p.id); const shownAfter = editing === "packaging" ? draftPackagingCover(p) : after?.totalCover ?? null;
            return <div className={`bp-shipment-plan-row ${(shownAfter ?? Infinity) < (settings.totalTargetWeeks ?? settings.targetWeeks) ? "is-warning" : "is-ok"}`} key={p.id}><span><b>{displayStyle(p.style)}</b><small>{p.type === "crates" ? "ארגזים" : "חביות"}</small></span><span>{coverLabel(before?.totalCover ?? null)}</span><span>{fmt(rec)}</span><span>{fmt(decided)}<small>אחרי החלטה: {coverLabel(shownAfter)}</small></span></div>;
          })}
        </div>
      ...
      <article className="bp-week-rec-card">
        ...
        <div className="bp-actions">{editing === "brew" ? <><button type="button" onClick={addBrew}>+ הוסף בישול</button><button disabled={busy} onClick={saveBrews}>שמירת החלטת הבישול</button><button onClick={() => setEditing(null)}>ביטול</button></> : <><button type="button" disabled={disabled || busy} onClick={addBrew}>+ הוסף בישול</button><button disabled={disabled || busy} onClick={() => beginEdit("brew")}>עריכת הבישולים</button></>}</div>
      </article>
    </div>
  </section>;
}
