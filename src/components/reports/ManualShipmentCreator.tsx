    ...
    return (
        <div className="modal-overlay" onClick={() => !saving && onClose()}>
            <section
                ...
                dir="rtl"
                style={{ width: "min(760px, calc(100vw - 24px))", maxHeight: "88vh", overflow: "auto" }}
                onClick={(event) => event.stopPropagation()}
            >
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                    <div>
                        <h3 style={{ marginBottom: 4 }}>תעודת משלוח חדשה</h3>
                        <p style={{ marginTop: 0 }}>בחר פריטים מהקטלוג או הוסף פריט חופשי.</p>
                    </div>
                    <button type="button" onClick={onClose} disabled={saving} aria-label="סגור">
                        <X size={20} />
                    </button>
                </div>
                        ...
                        <div key={line.id} style={{ border: "1px solid #dbe3ec", borderRadius: 12, padding: 12, display: "grid", gap: 10 }}>
                            <label className="spec-field">
                                ...
                                <select
                                    ...
                                    onChange={(event) => selectCatalog(line.id, event.target.value)}
                                >
                                    <option value="">בחר מהקטלוג</option>
                                    {catalog.map((option) => (
                                        <option key={`${option.sku}-${option.itemType}`} value={`${option.sku}__${option.itemType}`}>
                                            {option.sku} · {option.displayText}
                                        </option>
                                    ))}
                            ...
                            {line.source === "manual" && (
                                <>
                                    <label className="spec-field">
                                        <span className="spec-field-label">מק״ט</span>
                                        <input className="spec-input" value={line.sku} onChange={(event) => updateLine(line.id, { sku: event.target.value })} />
                                    </label>
                                    <label className="spec-field">
                                        <span className="spec-field-label">תיאור פריט</span>
                                        <input className="spec-input" value={line.description} onChange={(event) => updateLine(line.id, { description: event.target.value })} />
                                    </label>
                                </>
                            )}

                            {line.source === "catalog" && line.description && (
                                <div style={{ fontSize: 14 }}>
                                    <strong>{line.sku}</strong> · {line.description}
                                </div>
                            )}

                            <div style={{ display: "flex", gap: 10, alignItems: "end" }}>
                                <label className="spec-field" style={{ flex: 1 }}>
                                    <span className="spec-field-label">כמות</span>
                                    <input
                                        className="spec-input"
                                        type="number"
                                        min={1}
                ...
                <button type="button" style={{ marginTop: 12 }} onClick={() => setLines((current) => [...current, newLine()])} disabled={saving}>
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 18, justifyContent: "flex-end" }}>
                    ...
                    <button type="button" className="btn-primary" onClick={() => void save()} disabled={saving}>
                        {saving ? <BeerLoader message="יוצר תעודה..." size="spinner" /> : "צור תעודת משלוח"}
                    </button>
                </div>
            </section>
        </div>
