...
function sortPalletsInFlatZone(pallets: Pallet[]): Pallet[] {
    if (pallets.some((p) => typeof p.orderInZone === "number")) {
        return [...pallets].sort(
            (a, b) => (a.orderInZone ?? Number.MAX_SAFE_INTEGER) - (b.orderInZone ?? Number.MAX_SAFE_INTEGER),
        );
    }
        ...
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-box zone-move-modal" onClick={(e) => e.stopPropagation()} dir="rtl">
                ...
                <div className="zone-move-modal-footer">
                    ...
                    <button className="modal-save-btn" onClick={handleMove} disabled={moving}>
...
function ZoneListItem({
    ...
    return (
        <article
            className={`zone-tray-item ${beerStyleClass(pallet.beerStyle).className} ${selected ? "selected" : ""} ${marked ? "marked-for-shipment" : ""}`}
        >
            {bulkMode && (
                <label className="zone-bulk-check" onClick={(e) => e.stopPropagation()}>
                    <input type="checkbox" checked={selected} onChange={() => onToggleBulk(pallet.id)} />
                </label>
            )}
            ...
            <div className="zone-tray-item-actions" onClick={(e) => e.stopPropagation()}>
                {!bulkMode && (
                    <div className="zone-reorder-buttons" title="שינוי סדר">
                        <button type="button" className="zone-reorder-button" onClick={() => onMoveUp(pallet.id)} disabled={isFirst} title="הזז למעלה" aria-label="הזז למעלה">
                            <LayerArrowUp size={16} />
                        </button>
                        <button type="button" className="zone-reorder-button" onClick={() => onMoveDown(pallet.id)} disabled={isLast} title="הזז למטה" aria-label="הזז למטה">
                            <LayerArrowDown size={16} />
                        </button>
                    </div>
                )}

                {!bulkMode && (
                    <button className="zone-primary-action" onClick={() => onSelectForPlacement(pallet)}>
                        שבץ במפה
                    </button>
                )}
                ...
                {!bulkMode && (
                    <button
                        type="button"
                        className={marked ? "zone-primary-action" : "zone-secondary-action"}
                        onClick={() => void onToggleShipmentMark(pallet)}
                        disabled={markingShipment}
                    >
                        <Check size={14} />
                        {markingShipment ? "מעדכן..." : marked ? "בטל סימון משלוח" : "סמן למשלוח"}
                    </button>
                )}

                <button className="zone-secondary-action" onClick={() => onEditPallet(pallet)}>
                    <Pencil size={14} /> עריכה
                </button>

                {!bulkMode && (
                    <>
                        <button className="zone-secondary-action" onClick={() => onOpenMoveModal(pallet)}>
                            <ArrowRightLeft size={14} /> העבר
                        </button>
                        <button className="zone-danger-action" onClick={() => onDeletePallet(pallet)} title="מחק משטח">
                            <Trash2 size={14} /> מחק
                        </button>
                    </>
                )}
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-box zone-move-modal" onClick={(e) => e.stopPropagation()} dir="rtl">
                <div className="modal-header-row">
                    <div>
                        <span className="modal-kicker">העברה מרובה</span>
                        <h3>לאן להעביר?</h3>
                    </div>
                    <button className="modal-x" onClick={onClose} disabled={moving}>×</button>
                </div>
...
export function ZoneTray({
    ...
    function selectAll() {
        pallets.forEach((p) => {
            if (!selectedPalletIds.has(p.id)) onToggleBulk(p.id);
        });
    }
    ...
    async function movePalletUp(palletId: string) {
        ...
        try {
            setMovingOrderId(palletId);
            await reorderPalletsInZone(orderedIds);
        } finally {
    ...
    async function toggleShipmentMark(pallet: Pallet) {
        ...
        try {
            setMarkingShipmentId(pallet.id);
            await setMarkedForShipment(pallet.id, !pallet.markedForShipment);
        } catch (error) {
            setShipmentMarkError(error instanceof Error ? error.message : "עדכון סימון המשטח נכשל");
        } finally {
    ...
    return (
        <div className="zone-tray">
            <div className="zone-tray-header">
                ...
                {!bulkMode ? (
                    <button className="bulk-mode-btn" onClick={onStartBulk} disabled={pallets.length === 0}>בחר כמה להעברה</button>
                ) : (
            ...
            {bulkMode && (
                <div className="bulk-action-bar">
                    <div className="bulk-selection-info">
                        ...
                        <button type="button" onClick={allSelected ? clearAll : selectAll} className="bulk-select-all-btn">
                            {allSelected ? "בטל בחירת הכל" : "בחר הכל"}
                        </button>
                    </div>
                    <button disabled={selectedPalletIds.size === 0} onClick={() => setShowBulkMoveModal(true)}>העבר נבחרים</button>
                </div>
            )}

            {pallets.length === 0 ? (
                <div className="zone-tray-empty">אין משטחים באזור הזה כרגע.</div>
            ) : (
                <div className="zone-tray-list">
                    {displayPallets.map((p, index) => (
                        <ZoneListItem
                            key={p.id}
                            pallet={p}
                            ...
                            bulkMode={bulkMode}
                            isFirst={index === 0}
                            isLast={index === displayPallets.length - 1}
                            markingShipment={markingShipmentId === p.id}
                            onSelectForPlacement={onSelectForPlacement}
                            onToggleBulk={onToggleBulk}
                            onToggleShipmentMark={toggleShipmentMark}
                            onEditPallet={onEditPallet}
                            onOpenMoveModal={setMovingPallet}
                            onDeletePallet={onDeletePallet}
                            onMoveUp={movePalletUp}
                            onMoveDown={movePalletDown}
                        />
                    ))}
                </div>
            )}
            ...
            {movingPallet && (
                <ZoneMoveModal pallet={movingPallet} onMove={onMoveZone} onClose={() => setMovingPallet(null)} />
            )}

            {showBulkMoveModal && (
                <BulkZoneMoveModal
                    count={selectedPalletIds.size}
                    currentZone={pallets[0]?.zone ?? "pending"}
                    onMove={onBulkMove}
...
export function PendingTray(props: Omit<ComponentProps<typeof ZoneTray>, "title" | "hint">) {
    return (
        <ZoneTray
            ...
            hint="בחר משטח כדי לשבץ אותו במקרר, או הפעל בחירה מרובה להעברת מספר משטחים. התוקף והסימון למשלוח מוצגים על כל משטח."
        />
    );
}
...
export function BottleRoomTray(props: Omit<ComponentProps<typeof ZoneTray>, "title" | "hint">) {
    return (
        <ZoneTray
            {...props}
            title="חדר בקבוקים"
            hint="משטחים שנמצאים פיזית בחדר הבקבוקים. התוקף והסימון למשלוח מוצגים על כל משטח."
        />
    );
}
