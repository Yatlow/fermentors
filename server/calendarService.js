...
function syncCalendarToFirestore() {
  ...
  try {
    ...
    Logger.log(
      "Calendar sync complete: " + savedCount + " event(s), shipment queues=" +
      shipmentQueueSync.syncedWeeks
    );
    return { savedCount: savedCount, shipmentPlanningQueue: shipmentQueueSync };
